...
void DmaEthMonitor::initialize() {
    ...
    // s2e()->getCorePlugin()->onTranslateInstructionStart.connect(sigc::mem_fun(*this,
    // &DmaEthMonitor::onTranslateInst));
}
    ...
    if (address < 0x43000000 && address > 0x40000000) {
        ...
        if (address == 0x42470060 && concreteValue & 0x1) {
            ...
            if (concreteValue & 0x1) { // MB
                getDebugStream(state) << " set the MB in MACMIIAR: " << hexval(concreteValue) << " pc "
                                      << hexval(state->regs()->getPc()) << "\n";
                plgState->set_RegState(RegState::MACMIIAR_MB, true);
            }
            if (concreteValue & (uint64_t) PHYReg::PHY_BSR) {
                getDebugStream(state) << " set the PHY_BSR in MACMIIAR: " << hexval(concreteValue) << " pc "
                                      << hexval(state->regs()->getPc()) << "\n";
                plgState->set_RegState(RegState::MACMIIAR_PHY_BSR, true);
            }
            if (concreteValue & 0x8000) { // DP83848_BCR_SR
                getDebugStream(state) << " set the SR in MACMIIAR: " << hexval(concreteValue) << " pc "
                                      << hexval(state->regs()->getPc()) << "\n";
    ...
    if (address == RCC_CR) {
        if (concreteValue & 0x10000) { // when HSE on read HSE need to be HSE rdy
            *return_value = concreteValue | HSERDY;
            getDebugStream(state) << "HSE RDY when HSE on " << *return_value << "\n";
        }
        if (plgState->get_RegState(RegState::PLL)) {
            *return_value = concreteValue | PLLRDY;
            getDebugStream(state) << "PLL RDY when PLL on" << *return_value << "\n";
        }
        ...
        *return_value = concreteValue | 0x30000;
        getDebugStream(state) << "ODRDY&ODSWRDY when on" << *return_value << "\n";
    } else if (address == RCC_CFGR) { // get SW in CFGR
        *return_value = concreteValue | (plgState->get_CFGR_SW_val() << 2);
        getDebugStream(state) << " SWS is same to SW in CFGR: value " << *return_value << "\n";
    } else if (address == ETH_DMABMR) {
        ...
        if (plgState->get_RegState(RegState::DMABMR_SR)) { // SR is set;
            *return_value = concreteValue & ((uint64_t) -2);
            getDebugStream(state) << " return the value with clear the SR in DMABMR: " << *return_value << "\n";
        }
    } else if (address == ETH_MACMIIAR) {
        if (plgState->get_RegState(RegState::MACMIIAR_MB)) {
            *return_value = concreteValue & ((uint64_t) -2);
            getDebugStream(state) << " return the value with clear the MB in MACMIIAR: " << *return_value << " pc "
                                  << hexval(state->regs()->getPc()) << "\n";
        }
        // *return_value = concreteValue & ~0x8000;
        // getDebugStream(state) << " return with clear the SR in MACMIIAR: " << *return_value << " pc " <<
        // hexval(state->regs()->getPc()) <<  "\n"; if(plgState->get_RegState(RegState::MACMIIAR_DP83848_BCR_SR)) {
        //     *return_value = concreteValue & ~0x8000;
        //     getDebugStream(state) << " return the value with clear the SR in MACMIIAR: " << *return_value << "\n";
        // }
        ...
        if (plgState->get_RegState(RegState::MACMIIAR_PHY_BSR)) {
            *return_value = concreteValue | (PHY_LINKED_STATUS | PHY_AUTONEGO_COMPLETE);
            getDebugStream(state) << " return in MACMIIDR : " << *return_value << " pc "
                                  << hexval(state->regs()->getPc()) << "\n";
        }
    } else if (address == ETH_DMARDLAR) {
        if (plgState->get_RegState(RegState::SRL)) {
            getDebugStream(state) << "get RxDesc list address" << concreteValue << " pc "
                                  << hexval(state->regs()->getPc()) << "\n";
            // prepare_frame(state,concreteValue);
        }
    } else if (address == ETH_DMASR) {
        getDebugStream(state) << "read DMASR and emit the Fuzz pc " << hexval(state->regs()->getPc()) << "\n";
        ...
        for (i = 0; i < v.size(); i++) {
            if (address == v[i] && concreteValue & 0x80000000) {
                *return_value = concreteValue & ~0x80000000;
                getDebugStream(state) << " return without own in RxDesc: " << *return_value << " pc "
                                      << hexval(state->regs()->getPc()) << "\n";
            }
        }
    }
...
void DmaEthMonitor::read_prepare_RxDesc(S2EExecutionState *state, uint64_t address) {
    ...
    while (1) {
        ...
        getDebugStream(state) << " get the descriptor[" << i << "] address " << hexval(RxDescAddr) << "\n";
        ...
        // LOG OUT
        getDebugStream(state) << " get the EthBuf [" << i << "] address "
                              << hexval(get_reg_value(state, RxDescAddr + 8)) << " length " << hexval(DESC1 & 0x1FFF)
                              << "\n";
        i++;
    }

    getDebugStream(state) << i << " Descriptors have been found"
                          << "\n";
    ...
    getDebugStream(state) << "set the frame for RxDesc: address " << hexval(plgState->EthBufAddr[0]) << " length "
                          << plgState->get_FL() << "\n";
}
...
//         s2e()->getDebugStream() << "check the func of exit : " << func_name << " address " << hexval(pc) << "\n";
...
//     s2e()->getDebugStream() << "time to  exit : " << hexval(pc) << "\n";
//     exit(0);
// }
